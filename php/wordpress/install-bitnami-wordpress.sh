...
if [ "$FORCE" = true ]; then
    ...
    wp db reset --path=$WORDPRESS_PATH --yes
...
if ! wp core is-installed --path=$WORDPRESS_PATH; then
    ...
    wp core install --url="$URL" --title="$TITLE" --admin_user="$ADMIN_USER" --admin_password="$ADMIN_PASSWORD" --admin_email="$ADMIN_EMAIL" --locale=en_US --skip-email --path=$WORDPRESS_PATH
...
sudo bash $ROOT_PATH/php/wordpress/install-bitnami-wordpress-themes.sh
sudo bash $ROOT_PATH/php/wordpress/install-bitnami-wordpress-plugins.sh
