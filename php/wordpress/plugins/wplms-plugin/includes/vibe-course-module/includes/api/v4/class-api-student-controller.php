...
if ( ! class_exists( 'BP_Course_Rest_Student_Controller' ) ) {
	...
	class BP_Course_Rest_Student_Controller extends BP_Course_New_Rest_Controller {
		...
		public function register_routes() {
			...
			register_rest_route( $this->namespace, '/'.$this->type.'/allCategories', array(
				'methods'                   =>  'POST',
			...
			register_rest_route( $this->namespace, '/' . $this->type.'/courseButtons', array(
				array(
					'methods'                   =>  'POST',
					'callback'                  =>  array( $this, 'course_buttons' ),
					'permission_callback' => array( $this, 'get_student_permissions_check' ),
				),
			));
		...
		function get_student_courses($request){
			...
			if(!empty($courses_with_types) || !empty($args['post__in'])){
				...
				$query_args = apply_filters('wplms_mycourses',$query_args,$this->user->id);
				...
				if($course_query->have_posts()){
					...
					while($course_query->have_posts()){
						...
						$_course_data = array(
							...
							'excerpt'				=> nl2br($post->post_excerpt),
							'description'			=> apply_filters('the_content',$post->post_content),
							'user_progress'         => empty($progress)?0:intval($progress),
							'courseCat'				=> wp_get_object_terms($post->ID,'course-cat',['fields'=>'ids']),
						...
						$courses[]=apply_filters('wplms_student_course_data',$_course_data,$this->user->id);
		...
		function get_course_categories($request){

			$args = json_decode($request->get_body(),true);
			$terms = get_terms(['taxonomy'=>'course-cat','fields'=>'id=>name']);
			if(!empty($terms)){
				$newterms=[['value'=>'','label'=>__('Select a category','wplms')]];
				foreach($terms as $id=>$name){
					$newterms[]=['value'=>$id,'label'=>$name];
				}
				$return = ['status'=>1,'terms'=>$newterms];
			}else{
				$return = ['status'=>0];
			}
		...
		function get_course_tabs($request){
			...
			if(function_exists('bbpress')){
				...
				if(!empty($forum_id) && is_numeric($forum_id)){
					$tabs['course_forum'] = _x('Discussion','student course view in pwa','wplms');
				}
			}			
		...
		function course_buttons($request){
			$body = json_decode($request->get_body(),true);
			$course_ids = $body['ids'];
			$return = array('status'=>1,'courses'=>array());
			if(!empty($course_ids)){
				foreach ($course_ids as $key => $c) {
					$return['courses'][(string)$c] = $this->get_course_button_data($c,$body);
				}
			}
		...
		function course_button($request){
			$body = json_decode($request->get_body(),true);
			$course_id = $body['id'];
			return new WP_REST_Response( $this->get_course_button_data($course_id,$body), 200 );
		}

		function get_course_button_data($course_id,$body){
				...
				if($time > time()){
					...
					if( !empty($free) && $free=='S'){
				        ...
				        if($t){
				        	...
				        	$return['text'] = ((!empty($GLOBALS) && !empty($GLOBALS['wplms_course_enroll_error']))?$GLOBALS['wplms_course_enroll_error']:'');
				        	$return['link'] = get_permalink($course_id);
				        }
					}else{
			...
			return apply_filters('wplms_course_button_rest',$return,$this->user,$course_id);
		}
