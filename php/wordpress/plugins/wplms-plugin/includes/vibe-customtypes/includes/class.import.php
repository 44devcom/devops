...
class wplms_import{
	...
	function generate_form(){
		echo '	<h3>'.__('IMPORT SETTINGS','wplms').'</h3>
			<form method="post" enctype="multipart/form-data">
			    '.sprintf(__('Select File to upload [ Maximum upload size %s MB(s) ]','wplms'),$this->getmaxium_upload_file_size()).'<br />
			    <input type="file" name="upfile" id="fileToUpload"><br />
			    <input type="submit" value="'.__('Upload File','wplms').'" name="import" class="button-primary">';
	...
	function process_upload(){
		...
		try {
		    ...
		    if (
		        ...
		        throw new RuntimeException(__('Invalid parameters.','wplms'));
		    }
		    ...
		    switch ($_FILES['upfile']['error']) {
		        ...
		        case UPLOAD_ERR_NO_FILE:
		            throw new RuntimeException(__('No file sent.','wplms'));
		        case UPLOAD_ERR_INI_SIZE:
		        case UPLOAD_ERR_FORM_SIZE:
		            throw new RuntimeException(__('Exceeded filesize limit.','wplms'));
		        default:
		            throw new RuntimeException(__('Unknown errors.','wplms'));
		    }
		    ...
		    if (false === $ext = array_search(
		        ...
		        array(
		            'text/plain','text/csv','text/tsv','text/html','application/csv'
		        ),
		        true
		    )) {
		        throw new RuntimeException(__('Invalid file format.','wplms'));
		    }

		    if($this->process_csv($_FILES['upfile']['tmp_name'])){
		    	echo '<span class="file-success">'.__('Import Complete','wplms').'</span>';
		    }else{
		    	echo '<span class="file-error">'.__('Import Failed','wplms').'</span>';
		    };
		    ...
		    while ( ($data = fgetcsv($handle,1000,",") ) !== FALSE ) {
				...
				switch($table){
					case $wpdb->prefix.'postmeta':
						...
						if(in_array($field, $new_meta_keys)){
						...
						if(!isset($id_map[$data[1]])){ // User mapping already exists means user already exists
							...
							if($exists){
								...
								if(is_numeric($user_id)){
									...
									echo '<p class="file_error">'.__('Unable to Create user','wplms').' '.$login.'( '.$email.' )'.'</p>';
								}
							}
						}
					break;
    ...
    function parse_value($value,$field,$id_map){
    	if($field=='vibe_question_answer'){
    		return $value;
    	}
    	/*if($field=='vibe_post_video'){
    	...
    	if(strpos($value,'|')){
    		...
    		foreach($value_array as $k => $item){
    			...
    			if(strpos($item,'=>') !== false){
    				...
    				$value_array[$key_value[0]]=$key_value[1];
    				unset($value_array[$k]);
    			}else if(strpos($item,',')){
