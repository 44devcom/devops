...
class WPLMS_tips{
    ...
    */
	...
	function unit_key($key,$unit_id,$course_id){
		if(!empty($_POST['load_unit']) && $unit_id == $_POST['load_unit']){
			$units = bp_course_get_curriculum_units($course_id);
	...
	function wplms_mycourses($args){
		if(!empty($this->settings['default_my_courses_orderby'])){
			switch ($this->settings['default_my_courses_orderby']) {
				...
				case 'title':
					$args['orderby']='title';
					$args['order']=(!empty($this->settings['default_my_courses_order'])?$this->settings['default_my_courses_order']:'ASC');
					break;
				case 'start_date':
	...
	function save_unit_extra_settings($unit_id){
		if(!empty($_POST['extras']) && get_post_type($unit_id) == 'unit' && current_user_can('edit_posts')){
			$extras = json_decode(stripslashes($_POST['extras']));
	        if(is_array($extras) && isset($extras))
	        foreach($extras as $c){
	...
	function login_redirect($redirect_url,$request_url,$user){
		...
		if(($user instanceof WP_User)){
			...
			$flag=0;
			if (isset($user->allcaps['edit_posts']) && !empty($this->settings['instructor_login_redirect'])) {
