...
Class Vibe_Define_Shortcodes{
        ...
        switch($info){
            ...
            case 'instructor_avatar':
                ...
                foreach($authors as $instructor_id){
                    $html .= '<a href="'.bp_core_get_user_domain($instructor_id).'" class="course_instructor">'.bp_core_fetch_avatar( array( 'item_id' => $instructor_id,'type'=>'thumbnail', 'html' => true ) ).'<span>'.bp_core_get_user_displayname($instructor_id).'</span></a>';        
                }
                $html .='</div>';
            break;
            ...
            case 'certificate':
                ...
                $link =bp_get_sample_course_certificate_url($id);
                $html ='<a href="'.$link.'" class="bp_course_certificate_link">'.__('Preview Certificate','wplms').'</a>';
                add_action('wp_footer',[$this,'certificate_pop_link']);
                ...
                $b=bp_get_course_badge($id);
                 if(!empty($b)){
                    ...
                    if(!empty($badge_url)){
                        ...
                        $badge_url = 'https://placehold.co/60x60/f0f8ff/31343C';
                    }
                    $b_title = get_post_meta($id,'vibe_course_badge_title',true);
                    $html ='<a class="tip badge_popup_link" title="'.$b_title.'"><img src="'.$badge_url.'" title="'.$b_title.'" width="32px" data-course="'.get_the_title($id).'" /></a>';
                     add_action('wp_footer',[$this,'badge_pop_link']);
                }
            break;
            default:
                ob_start();
    ...
    function user_reviews($atts,$content=null){
        ...
        if(!empty($comments_query) && !empty($comments_query->comments)){
    ...
    function course_instructor_card($atts,$content=null){
        ...
        wp_enqueue_script('vibebp-members-actions',VIBEBP_PLUGIN_URL.'/assets/js/actions.js',array('wp-element','wp-data'),VIBEBP_VERSION);
    ...
    function course_featured($atts,$content=null){
        ...
        $html .= '<div class="'.$className.'" style="'.(is_numeric($width)?'width:'.$width.'px;':$width).';border-radius:'.(is_numeric($radius)?$radius.'px':$radius).'" ><img src="'.$image.'" /></div>';
        $html .= '</div>';
        return $html;
    }
...
/*-----------------------------------------------------------------------------------*/
    ...
    function wplms_registration_form($atts, $content = null){
        ...
        if(!empty($forms[$name]) && !empty($forms[$name]['fields'])){
