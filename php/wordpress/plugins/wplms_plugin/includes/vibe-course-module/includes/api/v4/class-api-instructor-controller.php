...
if ( ! class_exists( 'BP_Course_Rest_Instructor_Controller' ) ) {
	...
	class BP_Course_Rest_Instructor_Controller extends BP_Course_New_Rest_Controller {
		...
		public function register_routes() {
			...
			register_rest_route( $this->namespace, '/' . $this->type . '/deleteUnit', array(
				'methods'                   =>  'POST',
					...
					if($this->user_id){
						...
						if(!empty($course) && is_array($course) ){
							$course['instructor_permissions'] = array(
								'instructor_add_students' => apply_filters('wplms_instructor_add_students',$instructor_add_students,$course_id,$this->user_id),
								'instructor_change_status' => apply_filters('wplms_instructor_change_status',$instructor_change_status,$course_id,$this->user_id),
								'instructor_assign_badges' => apply_filters('wplms_instructor_assign_badges',$instructor_assign_badges,$course_id,$this->user_id),
								'instructor_extend_subscription' => apply_filters('wplms_instructor_add_students',$instructor_extend_subscription,$course_id,$this->user_id),
							);
						}
					}else{
				...
				if($is_instructor){
					if(is_array($members) && (count($members) > 0)){
						...
						foreach ($members as $member_id) {
							...
							$member_id=intval(apply_filters('add_member_to_course',$member_id));
		...
		function delete_unit($request){
			$args = json_decode($request->get_body(),true);
			if($this->user->id != get_post_field('post_author',$args['unit_id']) && !in_array('manage_options',array_keys($this->user->caps))){
				return new WP_REST_Response(array('status'=>0,'message'=>_x('Only quiz author can delete this quiz.','wplms')), 200);
			}

			wp_trash_post($args['unit_id']);
			return new WP_REST_Response(array('status'=>1,'message'=>_x('Unit deleted.','wplms')), 200);
		}
		...
		function get_instructor_questions($request){
			...
			if($questions_query->have_posts()){
				...
				$return=array('status'=>0,'columns'=>$columns,'message'=>__('No questions found.','wplms'));
			}
			return new WP_REST_Response($return, 200);
		}
