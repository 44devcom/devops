...
class WPLMS_Create_Course_Api{
            ...
            if(!empty($args['meta']) && count($args['meta'])){
                foreach ($args['meta'] as  $meta) {
                    ...
                    if(isset($meta['meta_value'])){
                        ...
                        $meta['meta_value'] = apply_filters('wplms_create_course_pre_save_meta_value',$meta['meta_value'],$meta,$id,$this->user);
                ...
                foreach ($body['course'] as $key => $tab) {
                    if(!empty($tab['fields'])){
                        foreach ($tab['fields'] as $ky => $field) {
                            ...
                            if(!empty($field['from']) && $field['from']=='post'){
    ...
    function create_element($request){
        ...
        $body = json_decode($request->get_body(),true);
        $cpt= esc_attr($request->get_param('cpt'));
        if($cpt=='assignment'){
            $cpt = 'wplms-assignment';
        }

        $allowed_cpts = apply_filters('wplms_create_element_cpts',array('unit','quiz','wplms-assignment','course','question','product','certificate'));
        $return = array();
        $return = array('status'=>false,'message'=>__('Not saved','wplms'),'allowed_cpts'=>$allowed_cpts);
        ...
        $manage_options = user_can($this->user->id,'manage_options');    
        $admin_approval = apply_filters('wplms_curriculum_create_element_approval',0,$cpt,$this->user->id,$body,$manage_options  );
        if($cpt == 'product' && $cpt =='question'){
            $admin_approval = 0;
        }
        ...
        if(empty($body['id'] )){
            ...
            if(empty($check_can_create)){
                $return = array('status'=>false,'message'=>__('Can not create questions','wplms'),'debug'=>$check_can_create.'-'.$cpt.'-'.$this->user->id);
                return new WP_REST_Response( $return, 200 ); 
            }
            $args = apply_filters('wplms_front_end_create_curriculum',array(
                'post_type' => $cpt,
                'post_title' => sanitize_textarea_field($body['post_title']),
                'post_content' => (!empty($body['post_content'])?wp_slash($body['post_content']):''),
                'post_status'=>'publish',
            ...
            if(!empty($body['post_title']) || !empty($body['post_content'])){
                ...
                $return = ['status'=>true,'message'=>__('Saved.','wplms')];
                add_filter('content_save_pre', 'wp_filter_post_kses');
                add_filter('content_filtered_save_pre', 'wp_filter_post_kses');
            }
        }
                    ...
                    if(isset($meta['meta_value'])){
                        ...
                        if($meta['meta_key'] == 'vibe_duration_parameter'){
                         ...
                         if($meta['meta_key'] == 'vibe_unit_attachments'){
                            $attachments = [];
                            if(!empty($meta['meta_value']) && is_array($meta['meta_value'])){
                                foreach ($meta['meta_value'] as $key => $mv) {
                                    if(is_array($mv)){
                                        $attachments[] = $mv['id'];
                                    }else{
                                        $attachments[] = $mv;
            ...
            if(user_can($this->user->id,'edit_posts')){
                if($cpt == 'groups'){
                    ...
                    if(function_exists('bbp_insert_forum')){
                        ...
                        if ( !empty($body['privacy']) && $body['privacy'] == 'private' ) {
                            bbp_privatize_forum( $new_forum_id );
                        } elseif ( !empty($body['privacy']) &&  $body['privacy'] == 'hidden' ) {
