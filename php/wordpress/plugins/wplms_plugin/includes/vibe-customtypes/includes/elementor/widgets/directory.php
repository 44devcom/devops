	...
	protected function render() {
        ...
        <script>
            ...
            document.addEventListener('single_course_loaded', (e) => {
            	runCoursebuttonEvents();
			});
			document.addEventListener('course_cards_loaded', (e) => {
            	runCoursebuttonEvents();
			});

			const runCoursebuttonEvents = () =>{
		...
		<div id="wplms_courses_directory" <?php  echo isset($settings['show_map'])?'class="with_map"':''?>>
			...
			<div class="wplms_courses_directory_main">
				...
				<div class="wplms_courses_directory <?php echo empty($settings['style'])?'':$settings['style'];?>">
					<?php 
				...
				<?php
				if( $course_query->found_posts > $settings['courses_per_page']['size']){
					if(!empty($settings['courses_pagination'])){
						?>
						<div class="wplms_courses_directory_pagination">
							<span>1</span>
							<a class="page_name">2</a>
							<?php
								$end = ceil($course_query->found_posts/$settings['courses_per_page']['size']);
								if($end === 3){
