...
class Wplms_Plugin_Register_Guten_Blocks {
	...
	function coursedirectory($settings){
		...
		wp_enqueue_script('vibebp-flatpickr',plugins_url('../../assets/js/flatpickr.min.js',__FILE__),array(),WPLMS_PLUGIN_VERSION,true);
		...
		<?php if(!empty($settings['per_row']) && !empty($settings['per_row']['size'])){?>
		<style>
			.wplms_courses_directory{grid-template-columns: repeat(auto-fit,minmax(<?php echo $settings['per_row']['size'].$settings['per_row']['unit'] ?>,1fr));}
	...
	function vibebp_block_assets(){
		...
		$fontsizeunit_options = apply_filters('vibe_bp_gutenberg_font_size_unit_options',array(
			'px','em','rem','pt','vh','vw','%'
		));
		$groups = array();
		$fields = array();
		...
		$settings = apply_filters('wplms_gutenberg_data',array(
			...
			'course_info_options'=>apply_filters('wplms_course_info_display_options',[
					...
					'instructor_avatar'=>__('Instructor avatar','wplms'),
