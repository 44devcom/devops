...
class VibeBp_Shortcodes{
    ...
    function vibebp_search($atts,$content = null){
    	...
    	<input type="text" name="search" value="'.(empty($_GET['search'])?'':esc_html($_GET['search'])).'" placeholder="'.(empty($atts['placeholder'])?__('Type to search...','vibebp'):$atts['placeholder']).'"/><span class="vicon vicon-search"></span>';
    ...
    function vibeapp_carousel($atts,$content=null){
    	...
    	<?php
    	} ?>.swiper-button-next:after, .swiper-button-prev:after{font-size:75% !important;}.swiper-container{overflow:hidden;}
    	</style><?php
        ...
        if(!empty($forms[$name])){
            ...
            */
            ...
            if(empty($settings['hide_username'])){
                $return .='<div class="vibebp_registration_field"><label>'.__('Username','wplms').'</label>'.'<input type="text" name="signup_username" placeholder="'.__('Login Username','wplms').'" required></div>';
            }

            $return .='<div class="vibebp_registration_field">'.'<label>'.__('Email','wplms').'</label>'.'<input type="email" name="signup_email" placeholder="'.__('Email','wplms').'" required></div>';

            $return .='<div class="vibebp_registration_field_wrap"><div class="password_fields"><div class="vibebp_registration_field">'.'<label '.(empty($settings['password_meter'])?:'for="signup_password"').'>'.__('Password','wplms').'</label>'.'<input type="password" '.(empty($settings['password_meter'])?'':'id="signup_password" class="form_field"').' name="signup_password" placeholder="'.__('Password','wplms').'" autocomplete="new-password"></div>';
            $return .='<div class="vibebp_registration_field">'.'<label>'.__('Confirm Password','wplms').'</label>'.'<input type="password" name="confirm_password" placeholder="'.__('Password','wplms').'" autocomplete="new-password"></div></div><div class="password_terms_wrapper"><label>'.__('Password Requirements','vibebp').'</label>
            ...
            if(!empty($member_types) && count($member_types) && !empty($settings['member_type']) && $settings['member_type'] == 'enable_user_member_types_select'){
                $return .= '<div class="vibebp_registration_field">'.'<label>'.__('Member type','vibebp').'</label><select name="member_type" id="member_type"><option value="">'._x('None','','wplms').'</option>';
            ...
            $return .='<div class="vibebp_registration_action">'.apply_filters('vibebp_registration_form_submit_button','<a href="#" class="submit_registration_form button">'.__('Register','wplms').'</a>').'</div>';
            $return .= '</ul></form></div><style></style>';
        }
        return $return;
    }
