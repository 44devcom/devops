...
class Wplms_Gutenberg_Theme{
    ...
    function extend_option($sections){
		...
		$sections[1]['fields'][4]['options'][2]= 'Fixed on Scroll down';
    ...
    function demo_style($args){
				$args=[
					'single_instructor' => array(
						...
						'src' => 'https://demos.wplms.io/oneinstructor/wp-content/plugins/wplms-demos/single_instructor.png',
						'version'=>'Gutenberg',
						'installation_type'=>array('instructor','academy'),
						'link'=>'https://demos.wplms.io/oneinstructor/',
						'plugins'=>array('vibebp','wplms_plugin','buddypress','woocommerce')
					),
					'main'=>array(
						'label'=>'Main',
						'src' => 'http://demos.wplms.io/main/wp-content/uploads/2023/06/main_demo.jpg',
						'version'=>'Gutenberg',
					...
					'startup'=>array(
						'label'=>'Startup',
						'src' => 'https://demos.wplms.io/startup/wp-content/plugins/wplms-demos/startup_demo.png',
						'version'=>'Gutenberg',
	...
	function scripts(){
		...
		if(empty(vibe_get_option('default_font'))){
			wp_enqueue_style('roboto_slab_font',get_stylesheet_directory_uri().'/css/default_fonts.css');	
		}
	...
	function instructor_courses($atts,$content=null){
        ...
        if(!empty($cids)){
			$course_ids = wp_list_pluck($cids,'value');
		}else{
			$course_ids=[99999];
		}
        ...
        $args = [
            ...
            'post_type'=>'course',
            'carousel_number'=>6,
            'include_post_ids'=>implode(',',$course_ids),
            'show_controls'=>1,
